  ...
  do
    ...
    if [ "$DOWNLOAD_BINARY" -eq "0" ]; then
      ...
      if [ ! -d "$tag" ]; then
        ...
        git clone https://github.com/bitpal/bitpal "$tag"
        pushd "$tag" || exit 1
        {
          git checkout "$tag"
          ...
          mkdir bin
          mv src/bitpald src/bitpal-cli src/bitpal-tx bin
          if [ "$FUNCTIONAL_TESTS" -eq "0" ]; then
            mv src/qt/bitpal-qt bin
          fi
        }
        popd || exit 1
      fi
      ...
      if [ -d "$tag" ]; then
        ...
        if [[ "$tag" =~ v(.*)(rc[0-9]+)$ ]]; then
            BIN_PATH="bin/bitpal-core-${BASH_REMATCH[1]}/test.${BASH_REMATCH[2]}"
        else
            BIN_PATH="bin/bitpal-core-${tag:1}"
        fi
        URL="https://www.bit-pal.net/$BIN_PATH/bitpal-${tag:1}-$PLATFORM.tar.gz"
        echo "Fetching: $URL"
        if ! curl -O -f $URL; then
            echo "Download failed."
            exit 1
        fi
        tar -zxf "bitpal-${tag:1}-$PLATFORM.tar.gz" -C "$tag" --strip-components=1 "bitpal-${tag:1}"
        rm "bitpal-${tag:1}-$PLATFORM.tar.gz"
      fi
    fi
  done
}
