...
// bitpal includes.
#include <..\src\script\bitpalconsensus.h>
...
int main()
{
    std::cout << "bitpalconsensus version: " << bitpalconsensus_version() << std::endl;
    ...
    auto op0Result = bitpalconsensus_verify_script_with_amount(pubKeyScript.data(), pubKeyScript.size(), amount, (const unsigned char*)&stream[0], stream.size(), 0, bitpalconsensus_SCRIPT_FLAGS_VERIFY_ALL, &err);
