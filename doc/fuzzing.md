# Fuzzing BitPal Core using libFuzzer

## Quickstart guide

To quickly get started fuzzing BitPal Core using [libFuzzer](https://llvm.org/docs/LibFuzzer.html):

```sh
$ git clone https://github.com/bitpal/bitpal
$ cd bitpal/
$ ./autogen.sh
$ CC=clang CXX=clang++ ./configure --enable-fuzz --with-sanitizers=address,fuzzer,undefined
# macOS users: If you have problem with this step then make sure to read "macOS hints for
# libFuzzer" on https://github.com/bitpal/bitpal/blob/master/doc/fuzzing.md#macos-hints-for-libfuzzer
$ make
...
[`process_message`](https://github.com/bitpal/bitpal/blob/master/src/test/fuzz/process_message.cpp) is a fuzzing harness for the [`ProcessMessage(...)` function (`net_processing`)](https://github.com/bitpal/bitpal/blob/master/src/net_processing.cpp). The available fuzzing harnesses are found in [`src/test/fuzz/`](https://github.com/bitpal/bitpal/tree/master/src/test/fuzz).
...
The project's collection of seed corpora is found in the [`bitpal-core/qa-assets`](https://github.com/bitpal-core/qa-assets) repo.

To fuzz `process_message` using the [`bitpal-core/qa-assets`](https://github.com/bitpal-core/qa-assets) seed corpus:

```sh
$ git clone https://github.com/bitpal-core/qa-assets
...
If you find coverage increasing inputs when fuzzing you are highly encouraged to submit them for inclusion in the [`bitpal-core/qa-assets`](https://github.com/bitpal-core/qa-assets) repo.

Every single pull request submitted against the BitPal Core repo is automatically tested against all inputs in the [`bitpal-core/qa-assets`](https://github.com/bitpal-core/qa-assets) repo. Contributing new coverage increasing inputs is an easy way to help make BitPal Core more robust.
...
with certain assembly code from BitPal Core's code. See [developer notes on sanitizers](https://github.com/bitpal/bitpal/blob/master/doc/developer-notes.md#sanitizers)
...
# Fuzzing BitPal Core using american fuzzy lop (`afl-fuzz`)

## Quickstart guide

To quickly get started fuzzing BitPal Core using [`afl-fuzz`](https://github.com/google/afl):
