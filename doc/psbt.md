# PSBT Howto for BitPal Core

Since BitPal Core 0.17, an RPC interface exists for Partially Signed BitPal
Transactions (PSBTs, as specified in
[BIP 174](https://github.com/bitpal/bips/blob/master/bip-0174.mediawiki)).
...
PSBT is an interchange format for BitPal transactions that are not fully signed
...
#### Multisig with multiple BitPal Core instances

Alice, Bob, and Carol want to create a 2-of-3 multisig address. They're all using
BitPal Core. We assume their wallets only contain the multisig funds. In case
they also have a personal wallet, this can be accomplished through the
multiwallet feature - possibly resulting in a need to add `-rpcwallet=name` to
the command line in case `bitpal-cli` is used.
