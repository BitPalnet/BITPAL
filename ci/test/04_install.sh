#!/usr/bin/env bash
#
# Copyright (c) 2018-2020 The BitPal Core developers
...
env | grep -E '^(BITPAL_CONFIG|BASE_|QEMU_|CCACHE_|WINEDEBUG|LC_ALL|BOOST_TEST_RANDOM|CONFIG_SHELL|(ASAN|LSAN|TSAN|UBSAN)_OPTIONS|TEST_PREVIOUS_RELEASES|PREVIOUS_RELEASES_DIR)' | tee /tmp/env
if [[ $HOST = *-mingw32 ]]; then
  DOCKER_ADMIN="--cap-add SYS_ADMIN"
elif [[ $BITPAL_CONFIG = *--with-sanitizers=*address* ]]; then # If ran with (ASan + LSan), Docker needs access to ptrace (https://github.com/google/sanitizers/issues/764)
...
if [ ! -d ${DIR_QA_ASSETS} ]; then
 if [ "$RUN_FUZZ_TESTS" = "true" ]; then
  DOCKER_EXEC git clone https://github.com/bitpal-core/qa-assets ${DIR_QA_ASSETS}
 fi
fi
