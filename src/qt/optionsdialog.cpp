    ...
    mapper(nullptr)
{
    ...
    ui->bitpalAtStartup->setVisible(false);
    ui->verticalLayout_Main->removeWidget(ui->bitpalAtStartup);
    ...
    ui->bitpalAtStartup->setToolTip(ui->bitpalAtStartup->toolTip().arg(PACKAGE_NAME));
    ui->bitpalAtStartup->setText(ui->bitpalAtStartup->text().arg(PACKAGE_NAME));

    ui->openBitPalConfButton->setToolTip(ui->openBitPalConfButton->toolTip().arg(PACKAGE_NAME));
    ...
    ui->unit->setModel(new BitPalUnits(this));
...
{
    /* Main */
    mapper->addMapping(ui->bitpalAtStartup, OptionsModel::StartAtStartup);
...
void OptionsDialog::on_openBitPalConfButton_clicked()
{
