...
static int AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc < 2 || HelpRequested(gArgs)) {
        ...
        std::string strUsage = PACKAGE_NAME " bitpal-tx utility version " + FormatFullVersion() + "\n\n" +
            "Usage:  bitpal-tx [options] <hex-tx> [commands]  Update hex-encoded bitpal transaction\n" +
            "or:     bitpal-tx [options] -create [commands]   Create hex-encoded bitpal transaction\n" +
            "\n";
...
static int CommandLineRawTx(int argc, char* argv[])
{
    ...
    try {
        ...
        if (!fCreateBlank) {
            ...
            // param: hex-encoded bitpal transaction
