...
const char * const BITPAL_CONF_FILENAME = "bitpal.conf";
 ...
 * https://github.com/bitpal/bitpal/pull/16097#issuecomment-514627823
 */
...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; //bitpal-tx using stdin
        std::string val;
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\BitPal
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\BitPal
    // Mac: ~/Library/Application Support/BitPal
    // Unix: ~/.bitpal
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "BitPal";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/BitPal";
#else
    // Unix
    return pathRet / ".bitpal";
#endif
#endif
}
...
bool ArgsManager::ReadConfigFiles(std::string& error, bool ignore_invalid_keys)
{
    ...
    const std::string confPath = GetArg("-conf", BITPAL_CONF_FILENAME);
...
std::string CopyrightHolders(const std::string& strPrefix)
{
    ...
    // Make sure BitPal Core copyright is not removed by accident
    if (copyright_devs.find("BitPal Core") == std::string::npos) {
        strCopyrightHolders += "\n" + strPrefix + "The BitPal Core developers";
    }
