// Copyright (c) 2014-2020 The BitPal Core developers
...
#define BITPAL_TIMEDATA_MAX_SAMPLES 200
...
void AddTimeData(const CNetAddr& ip, int64_t nOffsetSample)
{
    ...
    if (setKnown.size() == BITPAL_TIMEDATA_MAX_SAMPLES)
        return;
    if (!setKnown.insert(ip).second)
        return;

    // Add data
    static CMedianFilter<int64_t> vTimeOffsets(BITPAL_TIMEDATA_MAX_SAMPLES, 0);
